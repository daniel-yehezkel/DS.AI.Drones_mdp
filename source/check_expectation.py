...
def main():
    print(f"IDS: {ids}")
    problems_score = [0, 0, 0, 0, 0]
    num_iterations = 1000
    for i in range(num_iterations):
        print(i)
        for j, an_input in enumerate(small_inputs):
            try:
                my_problem = DroneStochasticProblem(deepcopy(an_input))
                my_problem.run_round()
            except EndOfGame:
                problems_score[j] += my_problem.score
                continue
    problems_score = [x / num_iterations for x in problems_score]
